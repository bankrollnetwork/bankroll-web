...
const tron_networks = {
    'mainnet': 'THVYLtjFbXNcXwDvZcwCGivS95Wtd4juFn',//'TWvNVtYeFgRtLkzzzLUahvSejQNTLqtTbu',//'TSHZ8qNCuAL2wacGocYzZxff9LZHWhRKYG',
...
function bindUI(){
    ...
    buyAmountInp.on("change paste keyup", (e) => {
        ...
        buyEstimate.text(`${numeral(amount * 0.50).format('0.000 a').toUpperCase()} DAY`)
    })
    ...
    transferAmountInp.on("change paste keyup", (e) => {
        ...
        transferEstimate.text(`${numeral(amount * 0.99).format('0.000 a').toUpperCase()} DAY`)
    })
}
...
async function showStats() {
    try {
        ...
        let totalTxs =  await credits.totalTxs().call()
        let players = await credits.players().call()
        let tronBalance = await credits.totalTronBalance().call()
        let totalSupply = await credits.totalSupply().call()
        let dividendBalance = await credits.dividendBalance_().call()
        let totalBNKR = await credits.totalBNKR().call()
...
async function showUserStats() {
    ...
    let difficulty = await bnkrMint.mintingDifficulty().call()
    let stats = await credits.statsOf(currentAddress).call()
    let balance = await credits.myTokens().call()
    let divs =  await credits.myDividends(true).call()
    let referrals = await credits.myReferrals().call()
    let level = await credits.fundingLevel(currentAddress).call()
    let efficiency =  await credits.fundingScale(currentAddress).call()
    let estimate = await credits.dailyEstimate(currentAddress).call()
    ...
    efficiency = (100 + efficiency.toNumber()) / difficulty
    ...
    $('#user-estimate').html(estimate ? `&#8776; ${formatSun(estimate)} TRX daily  <small>*</small>` : 'Reliable TRX and BNKR Earnings')
    $('#user-estimate-usdt').html(estimate ? `${approxStr} ${formatSun(estimate * prices.usdt)} USDT`:'')
    $('#user-return').text(`${numeral(efficiency).format('0.000')} %`)
    $('#user-rewards').text(` / ${numeral(rewards).format('0.000')} % Rewards`)
          ...
          <div class="card d-flex flex-row mb-3">
              <div class="d-flex flex-grow-1 min-width-zero">
                  <div class="card-body align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center">
                      ...
                      <p class="mb-1 text-white w-15 w-xs-100">DAY</p>
    ...
    var balanceTooltip = {
        ...
        callbacks: {
            label: function (tooltipItem, myData) {
                ...
                label = numeral(tooltipItem.yLabel).format('0.000 a').toUpperCase() + ' DAY' // parseFloat(tooltipItem.value).toFixed(2);
                return label;
            }
        }
    };
