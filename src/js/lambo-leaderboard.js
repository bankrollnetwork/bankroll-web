...
async function loadData(fingerprint = null, startTime){
    let requestObj = {size: 200, eventName: 'onTokenPurchase'}
    ...
    try {
        ...
        if (res.length) {
            ...
            _.forEach(res, value => {
                let account
                let from = tronWebLocal.address.fromHex(value.result.customerAddress)
                let timestamp = Math.floor(value.timestamp/1000)

                //console.log(from, value.result.bnkr, timestamp)

                if (players[from] == null){
                    account = {player:from, balance:parseFloat(value.result.tokens), tron:parseFloat(value.result.tron), timestamp:timestamp}
                    players[from] = account
                } else {
                    account = players[from]
                    account.balance += parseFloat(value.result.tokens)
                    account.tron += parseFloat(value.result.tron)
          ...
          <div class="card d-flex flex-row mb-3">
              <div class="d-flex flex-grow-1 min-width-zero">
                  <div class="card-body align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center">
                      ...
                      <p class="mb-1 w-15 w-xs-100">TRX</p>
          ...
          <div class="card d-flex flex-row mb-3">
              <div class="d-flex flex-grow-1 min-width-zero">
                  <div class="card-body align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center">
                      ...
                      <p class="mb-1 text-white w-15 w-xs-100">${formatSun(item.balance)}</p>
                      <p class="mb-1 w-15 w-xs-100">${formatSun(item.tron)}</p>
                  </div>
              </div>
          </div>
      </div>
