...
const tron_networks = {
    'mainnet': 'TMmWrjjKGRCdoUzmv6YUaov7mwxy1swDnq', //'TSHZ8qNCuAL2wacGocYzZxff9LZHWhRKYG',
...
function bindUI(){
    ...
    buyAmountInp.on("change paste keyup", (e) => {
        ...
        buyEstimate.text(`${numeral(amount * 0.90).format('0.000 a').toUpperCase()} Boost`)
    })

    sellAmountInp.on("change paste keyup", (e) => {
        let amount = Number.parseInt(sellAmountInp.val().trim())
        sellEstimate.text(`${numeral(amount * 0.90).format('0.000 a').toUpperCase()} BTT`)
    })

    transferAmountInp.on("change paste keyup", (e) => {
        let amount = Number.parseInt(transferAmountInp.val().trim())
        transferEstimate.text(`${numeral(amount * 0.99).format('0.000 a').toUpperCase()} Boost`)
    })
}
...
async function showWalletInfo() {
    try {
        ...
        $('#walletBalanceValue').text(formatSun(await credits.tokenBalance(currentAddress).call()))
    } catch(e){
        console.error(e)
    }
}
...
async function showStats() {
    try {
        ...
        $('#contractBalance').text(formatSun((await credits.totalTronBalance().call())))
        let supply = await credits.totalSupply().call()
        $('#totalSupply').text(formatSun(supply))
        $('#totalSupply-usdt').html(`${approxStr} ${formatSun(supply * prices.btt)} USDT `)
        let totalBNKR = (await credits.totalBNKR().call()).toNumber()
        $('#totalBNKR').text(formatSun(totalBNKR))
        //$('#poolBalance').text(formatSun((await tronWeb.trx.getBalance(donationPoolAddress))))
    }catch(e){}
}
...
async function showUserStats() {
    ...
    let referrals = (await credits.myReferrals().call()).toNumber()
    let level = (await credits.fundingLevel(currentAddress).call()).toNumber()
    let withdrawn = formatSun(stats[1].toNumber())
    let reinvested = formatSun(stats[12].toNumber())
    let efficiency = (100 / (await credits.fundingDivisor(currentAddress).call())) / difficulty;
    ...
    $('#user-bonus-usdt').html(`${approxStr} ${formatSun(balance * prices.btt)} USDT`)
    $('#user-vault').text(formatSun(divs))
    $('#user-vault-usdt').html(`${approxStr} ${formatSun(divs * prices.btt)} USDT`)
...
async function sell() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        $.notify({
            message: `<span class="text-white">The BTT from your sale will  be deposited to your DIVS</span>`
        }, {
            type: 'dark',
            delay: 5000,
            allow_dismiss: true
...
async function buy() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        credits.buy(referrer).send({tokenValue: tronWeb.toSun(amount), feeLimit: feeLimit, tokenId:'1002000' }).then(tx => {
          ...
          <div class="card d-flex flex-row mb-3">
              <div class="d-flex flex-grow-1 min-width-zero">
                  <div class="card-body align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center">
                      ...
                      <p class="mb-1 text-white w-15 w-xs-100">Boost</p>
    ...
    var balanceTooltip = {
        ...
        callbacks: {
            label: function (tooltipItem, myData) {
                ...
                label = numeral(tooltipItem.yLabel).format('0.000 a').toUpperCase() + ' Boost' // parseFloat(tooltipItem.value).toFixed(2);
                return label;
            }
        }
    };
