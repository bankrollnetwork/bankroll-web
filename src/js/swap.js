...
async function main() {
    ...
    //setInterval(watchSelectedWallet, 60000)
...
async function getFastVolume() {
    try {
        const response = await axios.get('https://bnkr-info.bankroll.network/volume/sun', {timeout:2000})
        volumeLoaded = true
        return parseInt(response.data)
    } catch (e){
        console.error('volume/sun', 'fail', e)
        return 0
    }
}
...
function bindUI() {
    ...
    let calcTokens = async (e) => {
        ...
        amount = (await swapContract.getTrxToTokenInputPrice(amount).call())
    ...
    let calcTRX = async (e) => {
        ...
        amount = tronWeb.toSun(amount)
        amount = (await swapContract.getTokenToTrxInputPrice(amount).call())
...
async function isSwapEnabled() {
    ...
    return allowance > BigInt(21e12) ? true : false
}
...
async function showWalletInfo() {
    try {
        ...
        console.error('showWallet fail',e)
    }
}
...
async function showStats() {
    try {
        ...
        totalTxs =  await swapContract.totalTxs().call(),
        players = await swapContract.providers().call(),
        tronBalance = await swapContract.tronBalance().call(),
        totalBNKR = await swapContract.tokenBalance().call(),
        price =  await swapContract.getTokenToTrxInputPrice(1e6).call(),
        supply = await swapContract.totalSupply().call()
        ...
        //$('#liquidity-usdt').html(`${approxStr} ${formatSun(tronBalance * prices.usdt + totalBNKR * prices.bnkr)} USDT`)
        $('#totalTxs').text(numeral(totalTxs).format('0,0.000 a').toUpperCase())
        $('#providers').text(players)
        ...
        //$('#totalSupply-usdt').html(`${approxStr} ${formatSun(totalBNKR * prices.bnkr)} USDT`)
    } catch (e) { 
...
async function showUserStats() {
    ...
    try {
        ...
        userTRX = await fastContract.balanceOf(currentAddress).call()
        userBNKR = await bnkr.balanceOf(currentAddress).call()
        userSwap = await swapContract.balanceOf(currentAddress).call()
        supply = await swapContract.totalSupply().call()
        userTXs = await swapContract.txs(currentAddress).call()
        ...
        if (userSwap > 0) {
            ...
            $('#user-swap-percentage').text(numeral((userSwap / supply) * 100n).format('0.000') + ' %')
            amount = (await swapContract.getLiquidityToReserveInputPrice(userSwap).call())
            console.log('sell-amount-estimate', amount)
            let trx_value = Number(amount[0])
            $('#user-balance-estimate').html(`<h5 class="color-theme-1 mr-2">Staked Value</h5> <h5><span class="text-white">${formatSun(trx_value)}</span> TRX + ` + `<span class="text-white">${formatSun(Number(amount[1]))}</span> BNKR = <span class="text-success">${formatSun(trx_value * 2)}</span> TRX</h5>`)
...
async function sell() {
    ...
    //let tokens = tronWeb.fromSun((await bnkr.balanceOf(currentAddress).call()))
...
async function addLiquidity() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        if (supply == 0) {
            ...
            let tokens = (await bnkr.balanceOf(currentAddress).call())

            let liquid_amount = (await swapContract.getTrxToLiquidityInputPrice(amount).call())
            balancedTokens = (await swapContract.getLiquidityToReserveInputPrice(liquid_amount).call())[1]
