...
const tron_networks = {
    'mainnet': 'TWkuzBQqzJpQFYoX4DXzMeswgeAqH7EkX2',//'TWvNVtYeFgRtLkzzzLUahvSejQNTLqtTbu',//'TSHZ8qNCuAL2wacGocYzZxff9LZHWhRKYG',
...
function bindUI(){
    ...
    transferAmountInp.on("change paste keyup", (e) => {
        ...
        transferEstimate.text(`${numeral(amount).format('0.000 a').toUpperCase()} Credits`)
    })
}
...
async function showStats() {
    try {
        ...
        let [totalTxs, players, tronBalance, totalSupply, dividendBalance, bnkrPoolBalance, totalBNKR] =
            await Promise.all(
                [
                    credits.totalTxs().call(),
                    credits.players().call(),
                    credits.totalTrxBalance().call(),
                    credits.totalSupply().call(),
                    credits.dividendBalance_().call(),
                    credits.swapBalance_().call(),
        ...
        $('#bnkrPool').text(formatSun(bnkrPoolBalance))
...
async function showUserStats() {
    ...
    claimEstimate = claimEstimate.toNumber()
    ...
    $('#user-estimate').html(estimate ? `&#8776; ${formatSun(estimate)} TRX / ${formatSun(claimEstimate)} BNKR daily` : 'Reliable TRX and BNKR Earnings')
    $('#user-estimate-usdt').html(estimate ? `${approxStr} ${formatSun(estimate * prices.usdt)} USDT / ${formatSun(claimEstimate * prices.bnkr)} USDT`:'')
...
async function claim() {
    if (!((await credits.myClaims().call()).toNumber())){
        showAlert('NO DIVS!!!','Slow down there buddy, you need to have some divs first!')
        return
    }

    credits.claim().send({callValue: 0, feeLimit: feeLimit }).then(tx => {
        console.log('withdraw', tx)
        refresh(tx)
    }).catch(e => {
        txError(e)
        ...
        if (res.length) {
            ...
            _.forEach(res, async value => {
                ...
                if (players[player] == null) {
                    ...
                    account.claims = parseFloat(value.result.claims)
                    players[player] = account
                } else {
                    account = players[player];
                    account.tokens = Math.max(account.tokens, parseFloat(value.result.tokens))
                    account.claims = Math.max(account.claims, parseFloat(value.result.claims))
                }
            })
          ...
          <div class="card d-flex flex-row mb-3">
              <div class="d-flex flex-grow-1 min-width-zero">
                  <div class="card-body align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center">
                      ...
                      <p class="mb-1 text-white w-15 w-xs-100">Credits</p>
    ...
    var balanceTooltip = {
        ...
        callbacks: {
            label: function (tooltipItem, myData) {
                ...
                label = numeral(tooltipItem.yLabel).format('0.000 a').toUpperCase() + ' Credits' // parseFloat(tooltipItem.value).toFixed(2);
                return label;
            }
        }
    };
