...
async function main() {
    ...
    //First UI render — sequential awaits so TronGrid's 6 req/sec limiter doesn't queue
    // a burst that times out and partial-loads the page.
    try {
        await mainLoop()
        await showStats()
    } catch (e) {
...
async function showUserStats() {
    try {
        ...
        try {
            ...
            // Older contracts don't have this method; treat as zero.
        }
        const totalSupply = await credits.totalSupply().call()
        ...
        // Percentage in basis points, then /100 to keep three decimals without float-dividing huge BigInts.
        const stakePercent = totalSupply > 0n ? Number(balance * 10000n / totalSupply) / 100 : 0
        const apy = balanceTrx > 0n ? Number(estimateTrx * 365n * 10000n / balanceTrx) / 100 : 0
...
async function sell() {
    let tokens = tronWeb.fromSun(await credits.myTokens().call())
