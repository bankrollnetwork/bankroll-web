const CHART_API_URL = 'https://api.bankroll.network/info_credits' //'http://localhost:3002/info'
...
async function main() {
    ...
    if (document.getElementById("categoryChart")) {
        ...
        const orderedPercentageData = [
            percentageData['BUY'].length ? percentageData['BUY'][0] : 0,
            percentageData['WITHDRAW'].length ? percentageData['WITHDRAW'][0] : 0,
            percentageData['TRANSFER'].length ? percentageData['TRANSFER'][0] : 0,
            percentageData['DONATION'].length ? percentageData['DONATION'][0] : 0
        ]

        const percentageLabels = ['Buys', 'Withdrawals', 'Transfers', 'Donations']

        const amountSum = _.reduce(orderedPercentageData, function (sum, obj) {
        ...
        const pieChartToolTip = {
            callbacks: {
                label: function (tooltipItem, myData, ind) {
                    var label = myData.datasets[tooltipItem.datasetIndex].label || '';

                    const rawData = (myData.datasets[tooltipItem.datasetIndex].data[tooltipItem.index] * amountSum) / 100;
                    if (label) {
                        label += ': ';
                    }
                    label = formatSun(rawData) + ' TRX';
                    return label;
                }
            }
        }

        let chartToolTipCategory = Object.assign({}, chartTooltip, pieChartToolTip)
    ...
    if (document.getElementById("barChartAll")) {
        ...
        let investDataAll = await $.ajax({
            url: CHART_API_URL + '/invest_data_all'
        });

        investDataAll = investDataAll.map(obj => {
            obj.y = Math.min(obj.y, 1e6)
            return obj
        })

        //console.log('investDataAll', investDataAll);

        let withdrawDataAll = await $.ajax({
            url: CHART_API_URL + '/withdraw_data_all'
        });

        withdrawDataAll = withdrawDataAll.map(obj => {
            obj.y = Math.min(obj.y, 1e6)
            return obj
        })
