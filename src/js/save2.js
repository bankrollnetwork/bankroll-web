...
const tron_networks = {
    'mainnet': 'TXu9wKz3yXB7FjYrmZa4NEeRyqkbLTafFR',//'THjY7rDKfjMiyCFMoCMCXdQAtRakD21RZQ',//'TXKaJj4ce3CMsvCo2FXMdkbzt5Rb4C16Mw',
...
async function showUserStats() {
    ...
    let totalSupply = (await credits.totalTronBalance().call()).toNumber()
...
async function sell() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        credits.unfreeze(tronWeb.toSun(amount), referrer).send({callValue: 0, feeLimit: feeLimit }).then(tx => {
...
async function buy() {
    ...
    if (amount <= 0 || !isFinite(amount) || amount === '') {
        ...
        var referrer = getReferrer()
        credits.freeze(referrer).send({callValue: tronWeb.toSun(amount), feeLimit: feeLimit}).then(tx => {
